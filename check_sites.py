...
def diff_to_image(diff_text, output_file, min_width=400, max_width=1200, line_height_pad=8):
    # 创建新的行列表，用于处理换行
    processed_lines = []
    ...
    line_height = (bbox[3] - bbox[1] if hasattr(font, 'getbbox') else bbox[1]) + line_height_pad
    ...
    if first_run:
        ...
        try:
            ...
            if old_content != content:
                ...
                # 发送更新通知
                caption = f"🔍🔍 内容更新: {url}\n时间: {timestamp}"
                bot.send_photo(chat_id=CHANNEL_ID, photo=open(diff_image_file, "rb"), caption=caption)
...
def main():
    if not BOT_TOKEN or not CHANNEL_ID or not ADMIN_USER_ID:
        raise ValueError("请设置 TELEGRAM_BOT_TOKEN, TELEGRAM_CHANNEL_ID 和 TELEGRAM_ADMIN_ID 环境变量")
